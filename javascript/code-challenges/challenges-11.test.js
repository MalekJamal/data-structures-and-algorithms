...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  const result = input.map(val=> val.filter(val2=>(val2%5===0 && typeof(val2)===typeof(1))).
  map(val3=>(Math.pow(2,val3))));
  return result;
};
...
let findShortest = (data) => {
  // Solution code here...
  let short=starWarsData.map(val=>(val.height)).sort((a,b)=>a-b)
  let name=starWarsData.find(val=> val.height === short[0])
return(name.name)
};
