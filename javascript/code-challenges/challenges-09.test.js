...
const checkValues = (obj, value) => {
  // Solution code here...
  let exist = Object.values(obj).includes(value);
return (exist)
...
const updateNumbers = (obj) => {
  ...
  const result2=[]
  result1.forEach(([key,value])=>{// destructuring
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.map(value=>{
    if(value.name === character){
      hasChildren = value.children ? value.children.length>0 : false
    }
  })
return hasChildren;
};
