...
const sortByChildren = (charArray) => {
  // Solution code here...
  const result= charArray.sort((a,b)=>{
...
a regular expression pattern to return true if the input contains a number, 
...
const citiesAtoJ = (arr) => {
  ...
  for(let i=0;i<arr.length;i++){
    let a= arr[i].match(regx)
   if(arr[i] == arr[i].match(regx))
    result.push(arr[i]);
 }
 return result;
};
