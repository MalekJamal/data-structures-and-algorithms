...
const typeNum = (arr) => {
  ...
  const arr2=arr.filter(item => 
    {if(typeof(item)=== typeof(5))return item;})
    return arr2;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const arr2= arr.filter(item=>{
    if(forbiddenValues.includes(item) !== arr.includes(item)){
      return item;
    }
...
Write a function named getBaseStatGreaterThan that, given the snorlaxData, below,
 and an integer as input, uses filter to return an array containing all stats with 
